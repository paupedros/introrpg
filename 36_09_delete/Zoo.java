...
public class Zoo {
    ...
    public Animal obteAnimalPerNom(String nom) throws SQLException {
        ...
        String sql = "SELECT ANIMALS.nom AS nom, ANIMALS.id AS id, CATEGORIES.nom AS cat_nom FROM ANIMALS, CATEGORIES WHERE CATEGORIES.id = ANIMALS.categoria AND ANIMALS.nom = '"
                + nom + "' ORDER BY ANIMALS.id LIMIT 1";
    ...
    public void canviaCategoria(Animal animal, Categoria categoria) throws SQLException {
        ...
        String sql = String.format("UPDATE ANIMALS SET categoria = %d WHERE id = %d", categoria.getId(),
                animal.getId());
        try (Statement st = conn.createStatement()) {
            st.executeUpdate(sql);
        }
    ...
    private boolean animalExists(Animal animal) throws SQLException {
        String sql = String.format("SELECT COUNT(id) FROM ANIMALS WHERE id = %d", animal.getId());
        try (Statement st = conn.createStatement()) {
            ResultSet rs = st.executeQuery(sql);
            rs.next();
            if (rs.getInt(1) > 0) {
